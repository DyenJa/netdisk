! function(t) {
	...
	t.extend(s.prototype, {
		_constructor: function() {
			...
			i.settings()[0]._responsive = this, t(e).on("resize.dtr orientationchange.dtr", r.util.throttle(function() {
				...
				i.off(".dtr"), t(i.table().body()).off(".dtr"), t(e).off("resize.dtr orientationchange.dtr"), t.each(n.s.current,
					function(t, e) {
		...
		_columnsVisiblity: function(e) {
			...
			for (n = 0, i = o.length; n < i; n++) {
				var f = o[n].columnIdx;
				"-" === a[f] && !s[f].control && s[f].minWidth && (p || h - s[f].minWidth < 0 ? (p = !0, a[f] = !1) : a[f] = !0,
					h -= s[f].minWidth)
			}
			var m = !1;
		...
		_classLogic: function() {
			var e = this,
				...
				a = function(t, i, r, a) {
					...
					if (r) {
						if ("max-" === r)
							for (d = e._find(i).width, l = 0, c = n.length; l < c; l++) n[l].width <= d && o(t, n[l].name);
						else if ("min-" === r)
							for (d = e._find(i).width, l = 0, c = n.length; l < c; l++) n[l].width >= d && o(t, n[l].name);
						else if ("not-" === r)
							for (l = 0, c = n.length; l < c; l++) n[l].name.indexOf(a) === -1 && o(t, n[l].name)
				...
				for (var r = e.className.split(" "), s = !1, o = 0, d = r.length; o < d; o++) {
					...
					t.each(n, function(t, e) {
						var n = e.name.split("-"),
							...
							o = l.match(r);
						o && (s = !0, o[2] === n[0] && o[3] === "-" + n[1] ? a(i, e.name, o[1], o[2] + o[3]) : o[2] !== n[0] || o[
							3] || a(i, e.name, o[1], o[2]))
					})
				}
				s || (e.auto = !0)
			}), this.s.columns = s
		...
		_detailsInit: function() {
			...
			t(n.table().body()).on("click.dtr mousedown.dtr mouseup.dtr", s, function(i) {
				if (t(n.table().node()).hasClass("collapsed") && n.row(t(this).closest("tr")).length) {
					...
					"click" === i.type ? e._detailsDisplay(o, !1) : "mousedown" === i.type ? t(this).css("outline", "none") :
						"mouseup" === i.type && t(this).blur().css("outline", "")
				}
			})
		},
		...
		_resizeAuto: function() {
			...
			if (this.c.auto && t.inArray(!0, t.map(n, function(t) {
				...
				if (a) {
					var d = t(a.cloneNode(!1)).appendTo(i),
						l = e.columns().footer().filter(function(t) {
							return e.column(t).visible()
						}).to$().clone(!1).css("display", "table-cell");
					t("<tr/>").append(l).appendTo(d)
				}
				t("<tr/>").append(o).appendTo(r), "inline" === this.c.details.type && t(i).addClass("dtr-inline collapsed"), t(
					i).find("[name]").removeAttr("name");
				var c = t("<div/>").css({
					width: 1,
					height: 1,
					overflow: "hidden"
		...
		_setColumnVis: function(e, n) {
			var i = this.s.dt,
				r = n ? "" : "none";
			t(i.column(e).header()).css("display", r), t(i.column(e).footer()).css("display", r), i.column(e).nodes().to$().css(
				"display", r)
		},
		...
		childRow: function(e, n, i) {
			return n ? t(e.node()).hasClass("parent") ? (e.child(i(), "child").show(), !0) : void 0 : e.child.isShown() ? (e.child(
				!1), t(e.node()).removeClass("parent"), !1) : (e.child(i(), "child").show(), t(e.node()).addClass("parent"), !0)
		},
		childRowImmediate: function(e, n, i) {
			return !n && e.child.isShown() || !e.responsive.hasHidden() ? (e.child(!1), t(e.node()).removeClass("parent"), !1) :
				(e.child(i(), "child").show(), t(e.node()).addClass("parent"), !0)
		},
		modal: function(e) {
			return function(i, r, s) {
				if (r) t("div.dtr-modal-content").empty().append(s());
				else {
					var o = function() {
							a.remove(), t(n).off("keypress.dtr")
						},
						a = t('<div class="dtr-modal"/>').append(t('<div class="dtr-modal-display"/>').append(t(
							'<div class="dtr-modal-content"/>').append(s())).append(t('<div class="dtr-modal-close">&times;</div>').click(
							function() {
								o()
		...
		listHidden: function() {
			return function(e, n, i) {
				var r = t.map(i, function(t) {
					return t.hidden ? '<li data-dtr-index="' + t.columnIndex + '" data-dt-row="' + t.rowIndex +
						'" data-dt-column="' + t.columnIndex + '"><span class="dtr-title">' + t.title +
						'</span> <span class="dtr-data">' + t.data + "</span></li>" : ""
				}).join("");
			...
			return e = t.extend({
				...
				function(n, i, r) {
					var s = t.map(r, function(t) {
						return '<tr data-dt-row="' + t.rowIndex + '" data-dt-column="' + t.columnIndex + '"><td>' + t.title +
							":</td> <td>" + t.data + "</td></tr>"
					}).join("");
	...
	}), s.version = "2.1.0", t.fn.dataTable.Responsive = s, t.fn.DataTable.Responsive = s, t(n).on("preInit.dt.dtr",
		function(e, n, i) {
			if ("dt" === e.namespace && (t(n.nTable).hasClass("responsive") || t(n.nTable).hasClass("dt-responsive") || n.oInit
					.responsive || r.defaults.responsive)) {
